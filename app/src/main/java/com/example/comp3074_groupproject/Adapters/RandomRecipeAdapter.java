...
import com.example.comp3074_groupproject.Models.Recipe;
...
public class RandomRecipeAdapter extends RecyclerView.Adapter<RandomRecipeViewHolder>{
    ...
    public RandomRecipeAdapter(Context context, List<Recipe> list, DetailClickListener listener) {
    ...
    public void onBindViewHolder(@NonNull RandomRecipeViewHolder holder, int position) {
        ...
        holder.textView_duration.setText(list.get(position).readyInMinutes+" Mints");
        Picasso.get().load(list.get(position).image).into(holder.imageView_food);

        holder.random_list_container.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                listener.onRecipeClicked(String.valueOf
                        (list.get(holder.getAdapterPosition()).id));
            }
        });
    }
